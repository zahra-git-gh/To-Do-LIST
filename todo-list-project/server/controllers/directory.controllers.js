...
const postDirectory = async (req, res) => {
  try {
    ...
    const isExist = directories.find(
      (directory) => directory.name === req.body.name
    );
    if (isExist) {
...
const updateDirectory = async (req, res) => {
  try {
    ...
    const updateDir = await directoryModel.findOneAndUpdate(
      { userId, _id },
      newData
    );
    res.status(201).json({
      msg: "directory updated",
      directory: { ...updateDir._doc, ...newData },
    });
  } catch (error) {
    res.status(500).json({ msg: error });
  }
